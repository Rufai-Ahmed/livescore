...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="lg:w-[calc(100%-0px)] flex flex-col items-center w-full mt-10">
      {/* Header section */}
      ...
      {/* Match info section */}
      {!isLoading ? (
        <Carousel className="max-w-[300px] lg:max-w-[900px]">
          <CarouselContent>
            {sessions.map((el: any, i: number) => (
              <CarouselItem key={i}>
                <div className="bg-gradient-to-r from-blue-500 to-red-500 rounded-lg p-4 text-red-200 w-[300px] md:w-[700px] mx-auto ml-5 lg:ml-0 min-h-[300px]">
                  <div className="flex justify-between items-center flex-wrap lg:flex-nowrap">
                    ...
                    <div className="flex flex-col justify-center text-center">
                      <h1 className="text-4xl font-bold">0 - 0</h1>
                      <p>
                        {moment(el?.startTime ? el?.startTime : "")
                          .format("LLLL")
                          .replace(",", " | ")}
                      </p>
                    </div>
      ...
      <div className="grid grid-cols-4 space-y-4 gap-4 mt-6">
        <Card title="Goal of the Day" content="Alfred N." />
        <Card title="Best Team Today" content="Henado FC" />
        <Card title="Top Goal Scorer Today" content="Isaac Adogbeji" />
        <Card title="Total Teams Today" content={teams.length.toString()} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-8 mt-6 gap-4">
          ...
          <div className="mt-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-baseline">
                ...
                <span className="text-sm text-gray-500 ml-1">/ $12,000</span>
              </div>
              <button className="text-xs text-blue-500">Edit</button>
            </div>
            <div className="w-full bg-gray-200 h-2 mt-1">
              <div className="bg-red-500 h-2" style={{ width: "62.2%" }}></div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-1 lg:col-span-5 overflow-auto min-h-[300px] p-4 border rounded-lg shadow-md bg-white">
          <div className="flex justify-between items-center mb-2">
            <h1 className="font-medium text-[25px]">Next Team to Play</h1>
            {/* <button
          ...
          <p className="text-sm text-gray-500 mb-4">
            If a team is not available to play, they will be moved 1 place down
            the list.
          </p>
                  ...
                  {["W", "D", "L"].map((status) => (
                    <button
                      ...
                      onClick={() =>
                        setNewTeam((prevTeam) => ({
                          ...prevTeam,
                          standings: prevTeam.standings.includes(status)
                            ? prevTeam.standings.filter((s) => s !== status)
...
const Card: React.FC<CardProps> = ({ title, content }) => (
  <div className="bg-white col-span-2 lg:col-span-1 p-4 rounded-lg shadow-md flex gap-5 items-center">
    <BiTrendingUp size={25} color="red" />
    <div>
